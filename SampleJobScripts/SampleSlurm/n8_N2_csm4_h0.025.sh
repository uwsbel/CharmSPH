...
let numProcs="($cpusPerNode * $numNodes) - numTasks" # Total number of processors available
...
echo "./charmrun +p"${numProcs}" ./charmsph +ppn "${ppn}" +commap 0,32 +pemap 1-31:33-63 -x "${X}" -y "${Y}" -z "${Z}" -h "${H}" -w "${W}" -id "${SimID}" -wp "${WP}" -t "${T}
./charmrun +p${numProcs} ./charmsph ++ppn ${ppn} +commap 0,32 +pemap 1-31:33-63 -x ${X} -y ${Y} -z ${Z} -h ${H} -w ${W} -id ${SimID} -wp ${WP} -t ${T}
